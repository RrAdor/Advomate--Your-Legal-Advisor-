  ...
  if (userData && authSection) {
    ...
    authSection.innerHTML = `
                    <div class="user-greeting">
                        ...
                        <span class="user-name-display">Welcome, ${sessionManager.getUserDisplayName()}</span>
    ...
    // Store user email in localStorage for hiring functionality (backward compatibility)
    ...
    if (!error && data) {
      ...
      authSection.innerHTML = `
                    <div class="user-greeting">
                        ...
                        <span class="user-name-display">Welcome, ${
                          data.name || "User"
                        }</span>
                        <a href="userProfile.html" class="back-to-profile">
                            <i class="fas fa-arrow-left"></i> Back to Profile
  ...
  if (data.length > 0) {
    ...
    data.forEach((lawyer) => {
      ...
      hireButton.style.cssText = `
            ...
            font-weight: 500;
          `;
      hireButton.onmouseover = () =>
        (hireButton.style.backgroundColor = "#219a52");
      hireButton.onmouseout = () =>
        (hireButton.style.backgroundColor = "#27ae60");
      ...
      console.log(
        "Using lawyer phone (Primary Key):",
        lawyerPhone,
        "Type:",
        typeof lawyerPhone
      );
...
async function hireLawyer(lawyerPhone, lawyerName) {
  ...
  if (checkError || !lawyerCheck) {
    console.error(
      "Lawyer not found with phone:",
      lawyerPhone,
      "Error:",
      checkError
    );
    alert(
      `Lawyer not found in database with phone: ${lawyerPhone}. Please try again.`
    );
    return;
  }

  console.log("Found lawyer:", lawyerCheck);
  console.log("Lawyer fields:", Object.keys(lawyerCheck));
  console.log("Lawyer ID:", lawyerCheck.id);
  console.log("Lawyer phone:", lawyerCheck.phone);

  const userData = sessionManager.getUserSession();
  console.log("Checking user session:", userData);

  if (!userData) {
    console.error("No user session found");
    alert("Please log in to hire a lawyer. No user session found.");
  ...
  try {
    ...
    if (currentUserError) {
      ...
      if (currentUserError.code === "PGRST116") {
        alert(
          "User not found in database. Please log in again or register a new account."
        );
        window.location.href = "login.html";
      } else {
        alert(
          `Error accessing user data: ${currentUserError.message}. Please try logging in again.`
        );
        window.location.href = "login.html";
      }
      return;
    ...
    console.log("Final hired lawyers data:", userDataCheck.hired_lawyers);

    // Get the user ID for junction table approach
    const { data: currentUser, error: currentUserIdError } = await supabase
      .from("users")
      .select("id")
      .eq("email", userData.email)
      .single();

    if (currentUserIdError || !currentUser) {
      console.error("Error getting user ID:", currentUserIdError);
      alert("Error accessing user information.");
      return;
    }
    ...
    // Check if lawyer is already hired (simple check using hired_lawyers field)
    const lawyerPhoneInt = parseInt(lawyerPhone);
    const isAlreadyHired = userData.hired_lawyers === lawyerPhoneInt;
    ...
    // For now, let's just use the simple fallback approach that we know works
    console.log("Hiring lawyer using simple approach...");

    try {
      // Update the user's hired_lawyers field with the lawyer's phone number
      const { data: updateResult, error: updateError } = await supabase
        .from("users")
        .update({ hired_lawyers: lawyerPhoneInt })
        .eq("email", userData.email)
        .select();

      if (updateError) {
        console.error("Error updating hired lawyer:", updateError);
        alert(`Failed to hire lawyer: ${updateError.message}`);
        return;
      }
      ...
      console.log("Lawyer hired successfully:", updateResult[0]);
    } catch (error) {
    ...
    alert(
      `Successfully hired ${lawyerName}! You can view them in your profile.`
    );
...
window.testHiredLawyersInit = async function () {
  ...
  try {
    ...
    alert(
      `Integer Schema Test:\n\nCurrent hired_lawyers: ${
        userData.hired_lawyers
...
window.resetHiredLawyers = async function () {
  ...
  const confirm = window.confirm(
    "Are you sure you want to reset your hired lawyer?"
  );
  if (!confirm) return;
